local tntctl  = require('tarantoolctl').new()
...
local function control_prepare_context(ctl, ctx)
    ...
    ctx.systemd = os.execute("systemctl 2>/dev/null | grep '\\-\\.mount' " ..
                             "1>/dev/null 2>/dev/null") == 0
    ctx.with_systemd = ctx.systemd and not ctx.usermode
...
local function wrapper_cfg_closure(ctx)
    ...
    return function(cfg)
        ...
        log.error("%s", cfg.background)
        log.error("%s", ctx.under_systemd)
